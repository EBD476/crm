...
$('#lightDataForm').submit(function(e) {
...
$('#removeRoomForm').submit(function(e) {
    ...
    e.preventDefault();
    var form = $(this);
    var roomID = form.serializeArray()[1].value;
...
$('.modal').on('show.bs.modal', function(e) {
   ...
   $(this).find("select").val('1');
    ...
    var moduleType = $(e.relatedTarget).data('type');
   $(this).find('#moduleType').val(moduleType);


   if ( $(e.relatedTarget).data('value')) {

       if ($(e.relatedTarget).data('value').length > 0) {
           $(this).find("#module-table").html('');
           $(this).find("#module-table2").html('');
       }

       for (i = 0; i<$(e.relatedTarget).data('value').length; i++)
       {

           lighModuleType = JSON.stringify($(e.relatedTarget).data('value')[i].type);

           $(this).find("#module-table").append('<tr><td>' +
               '                   <div class="form-group">\n' +
               '                                            <label for="normal-field" class="col-sm-2 control-label">Light Type</label>\n' +
               '                                            <div class="col-sm-3">\n' +
               '                                                <select name="lightType[]" id="state" data-placeholder="Select light type"\n' +
               '                                                        required="required" class="select2 form-control" name="room_type">\n' +
               '                                                    <option value="1"' + (lighModuleType == "\"1\"" ? "selected" : '') +'  >Normal</option>\n' +
               '                                                    <option value="2"' + (lighModuleType == "\"2\"" ? "selected" : '') +'>Dimmer</option>\n' +
               '                                                    <option value="3"' + (lighModuleType == "\"3\"" ? "selected" : '') +'>RGB</option>\n' +
               '                                                </select>\n' +
               '                                            </div>\n' +
               '            \n' +
               '                                            <label for="normal-field" class="control-label col-sm-2 " >Light Name</label>\n' +
               '                                                <div class="col-sm-4">\n' +
               '                                                   <input name="lightName[]" type="text" id="normal-field" ' +
               '                                                        class="form-control" placeholder="Type light name"' +
               '                                                         value= ' +  JSON.stringify($(e.relatedTarget).data('value')[i].name) +
               '                                               >\n </div>\n' +
               '            \n' +
               '                                                <div class="col-sm-1">\n' +
               '                                                    <button class="btn btn-primary remove-module"> ' +
               '                                                        <i class="fa fa-minus"></i> </button>\n' +
               '                                                </div>\n' +
               '                                        </div></td></tr>');

           $(this).find("#module-table2").append('<tr><td>' +
               '                   <div class="form-group">\n' +
               '                                            <label for="normal-field" class="control-label col-sm-2 " >Module Name</label>\n' +
               '                                                <div class="col-sm-8">\n' +
               '                                                   <input name="moduleName[]" type="text" id="normal-field" ' +
               '                                                        class="form-control" placeholder="Type module name"' +
               '                                                         value= ' + JSON.stringify($(e.relatedTarget).data('value')[i].name) +
               '                                               >\n </div>\n' +
               '            \n' +
               '                                                <div class="col-sm-1">\n' +
       ...
       $("#module-table tr:last").find('.remove-module')
           .addClass('add-module')
           .removeClass('remove-module')
           .html(' <i class="fa fa-plus"></i> ');

       $("#module-table2 tr:last").find('.remove-module')
           .addClass('add-omodule')
           .removeClass('remove-module')
           .html(' <i class="fa fa-plus"></i> ');


       var roomName = $(e.relatedTarget).data('value').room_name;
       var roomType = $(e.relatedTarget).data('value').room_type;

       $(this).find('input[name=roomName]').val(roomName);
       $(this).find('#select2').val(roomType).trigger('change');
...
$('.add-module').on('click', function(e) {
  ...
  //  $("#module-table tr:last").after('<tr><td><button class="btn btn-primary add-module" data-dismiss=""> <i class="fa fa-plus"></i> </button></td></tr>');

});

$("#module-table").on("click", ".add-module", function (e) {

    e.preventDefault();
    $("#module-table .add-module").html('<i class="fa fa-minus"></i>')
                                  .addClass('remove-module')
                                  .removeClass('add-module');

    $("#module-table tr:last").after('<tr><td>' +
        '                   <div class="form-group">\n' +
        '                                            <label for="normal-field" class="col-sm-2 control-label">Light Type</label>\n' +
        '                                            <div class="col-sm-3">\n' +
        '                                                <select name="lightType[]" id="state" data-placeholder="Select light type"\n' +
        ...
        '                                            <label for="normal-field" class="control-label col-sm-2 " >Light Name</label>\n' +
        '                                                <div class="col-sm-4">\n' +
        '                                                   <input name="lightName[]" type="text" id="normal-field" class="form-control" placeholder="Type light name">\n' +
        '                                                </div>\n' +
        '            \n' +
        '                                                <div class="col-sm-1">\n' +
...
$("#module-table2").on("click", ".add-omodule", function (e) {
    e.preventDefault();
    $("#module-table2 .add-omodule").html('<i class="fa fa-minus"></i>')
        .addClass('remove-module')
        .removeClass('add-omodule');

    $("#module-table2 tr:last").after('<tr><td>' +
        '                   <div class="form-group">\n' +
        '                                            <label for="normal-field" class="control-label col-sm-2 " >Module Name</label>\n' +
        '                                                <div class="col-sm-8">\n' +
        '                                                   <input name="moduleName[]" type="text" id="normal-field" class="form-control" placeholder="Type module name">\n' +
        '                                                </div>\n' +
        '            \n' +
        '                                                <div class="col-sm-1">\n' +
...
function createLightForm(){

    $("#module-table").html('<tr><td>' +
        '                   <div class="form-group">\n' +
        '                                            <label for="normal-field" class="col-sm-2 control-label">Light Type</label>\n' +
        '                                            <div class="col-sm-3">\n' +
        '                                                <select name="lightType[]" id="state" data-placeholder="Select light type"\n' +
...
function createModuleForm(){
    ...
    $("#module-table2").html('<tr><td>' +
        ...
        '            \n' +
        '                                                <div class="col-sm-1">\n' +
        '                                                    <button class="btn btn-primary add-omodule"> ' +
